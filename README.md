...
- **Volume awareness.** Small badges show when an item is on iCloud Drive, a network share, or an external drive. When a volume gets ejected, Monarch shows a friendly "Volume not mounted" state instead of a generic error — and refreshes automatically when the drive comes back. Shortcuts on disconnected drives or servers aren't lost either: they stay in your list as greyed-out rows (remove them if you want), spring back to life the moment the volume mounts, and Monarch never tries to connect to a server on its own — no launch stalls, no "server not available" dialogs.
